...
/* FNV-1a. A NULL pointer with a non-zero length cannot be reported through a
   return value that is itself a hash, so it yields the offset basis unchanged
   and hashes nothing. Callers that need the distinction check the pointer. */
